...
*Setup instructions for a Gitian build of obscuraX Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the obscuraX
...
- [Building obscuraX Core](#building-obscurax-core)
...
Clone the git repositories for obscuraX Core and Gitian.
...
in the obscuraX Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building obscuraX Core
----------------

To build obscuraX Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the obscuraX Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/obscurax/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/obscuraxpay/obscurax
...
For example:
```bash
URL=https://github.com/crowning-/obscurax.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit obscurax=${COMMIT} --url obscurax=${URL} ../obscurax/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit obscurax=${COMMIT} --url obscurax=${URL} ../obscurax/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit obscurax=${COMMIT} --url obscurax=${URL} ../obscurax/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the obscurax git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../obscurax/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/obscurax
SIGPATH=/some/root/path/obscurax-detached-sigs

./bin/gbuild --url obscurax=${BTCPATH},signature=${SIGPATH} ../obscurax/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/obscurax-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/obscurax-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/obscurax-osx-build.assert
```
...
[obscurax/gitian.sigs](https://github.com/obscuraxpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
