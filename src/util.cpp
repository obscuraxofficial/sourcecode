...
const char * const BITCOIN_CONF_FILENAME = "obscurax.conf";
const char * const BITCOIN_PID_FILENAME = "obscuraxd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "obscurax" is a composite category enabling all obscuraX-related debug output
                if(ptrCategory->count(std::string("obscurax"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\obscuraX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\obscuraX
    // Mac: ~/Library/Application Support/obscuraX
    // Unix: ~/.obscurax
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "obscuraX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/obscuraX";
#else
    // Unix
    return pathRet / ".obscurax";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty obscurax.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override obscurax.conf
